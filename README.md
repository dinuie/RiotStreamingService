...
In order to run this project, firstly you must ```clone``` the project, after this,  you must ```run``` the RiotApplication with an IDE(IntelliJ Idea, Visual Studio Code) and also open riot_react folder with an IDE and type ``` npm/yarn install``` then ``` npm start/yarn start``` in the terminal.
